...
for u in u_dict:
    ...
    vl_c=len(u_dict[u][v_last])/v_dict[v_last][2]
    ...
    r=[u, i, ul_dict[u], v_last]+map(str, [vl_s/float(s), vl_s, vl_m, vl_c, vl_t, vl_int, v_dict[v_last][0], v_dict[v_last][1], v_dict[v_last][2] ])
    r2=[0 for i in xrange(len(VL))]
    if v_last in VL: r2[VL[v_last]]=1
    r+=map(str, r2)
    print ','.join(r)
